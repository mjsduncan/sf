...
# average of fraction of significant probes are close to jpm result 
...
summary(unlist(lapply(jpmExp.slope[-c(19, 20)], function (x) (sum((x[, 3] <= .05 | x[, 3] >= .95) & x[, 1] > 0, na.rm = TRUE) / length(x)))))
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.01524 0.01835 0.02180 0.02220 0.02410 0.03799 
summary(unlist(lapply(jpmExp.slope[-c(19, 20)], function (x) (sum((x[, 3] <= .05 | x[, 3] >= .95) & x[, 1] < 0, na.rm = TRUE) / length(x)))))
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.01281 0.01582 0.02193 0.02343 0.02871 0.04047 
